...
This happens when a symbol set with the same identifier already exists but has different content. Common causes include:

1. **Xcode hasn't finished generating dSYMs** – The upload script runs before dSYM files are ready. Fix: Leave **Based on dependency analysis** enabled and add `$(DWARF_DSYM_FOLDER_PATH)/$(DWARF_DSYM_FILE_NAME)/Contents/Resources/DWARF/$(EXECUTABLE_NAME)` to the **Input Files** of the Run Script build phase.

2. **Rebuilding without code changes** – Minor non-determinism in the build can produce different content hashes. If you're re-uploading intentionally, pass `POSTHOG_CLI_EXTRA_ARGS="--skip-on-conflict"` to keep existing symbols, or `POSTHOG_CLI_EXTRA_ARGS="--force"` to overwrite them.

3. **Multiple build profiles uploading to the same project** – Different configurations (debug vs release) producing conflicting uploads. Consider using separate PostHog projects for each configuration.
