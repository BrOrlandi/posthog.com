...
Sessions group multiple [traces](/docs/ai-observability/traces) together using the `$ai_session_id` property.
...
Click any session to open its detail page. At the top, a title summarizes what the session is about – typically derived from the first user message. Below the title, the page displays the full conversation – all messages exchanged between users and the LLM across the session's traces.

Each conversation turn shows:

- **Input** – The user's message or prompt
- **Output** – The model's response
- **Turn metadata** – Latency, cost, timestamp, and error status

Messages that repeat across multiple traces (common when later traces include conversation history) are automatically deduplicated, so you see each message once in chronological order.

To inspect the underlying trace events for a turn, click **Show reasoning**. This reveals all generations, spans, and tool calls that produced that response.
...
When viewing a [trace](/docs/ai-observability/traces), you can see which session it belongs to, helping you understand the broader context of related traces.
